...
def _rounded_right_rectangle(draw: Any, xy: Tuple[int, int, int, int], radius: int, fill: Any = None, outline: Any = None, width: int = 1) -> None:
    """Zeichnet ein Rechteck mit nur rechts gerundeten Ecken.

    Pillow wirft bei manchen Versionen ValueError: "y1 must be greater than or equal to y0",
    wenn der Radius größer als die halbe Höhe ist. Darum wird der Radius hier sauber
    geklemmt und jede Teilform nur mit gültigen Koordinaten gezeichnet.
    """
    try:
    ...
    radius = min(radius, rect_w // 2, rect_h // 2)
    ...
    if fill is not None:
        # Linker und mittlerer Körper.
        if x2 - radius >= x1:
            draw.rectangle((x1, y1, x2 - radius, y2), fill=fill)
        # Rechter Mittelstreifen nur zeichnen, wenn y2-r >= y1+r. Genau diese Prüfung
        # verhindert den gemeldeten Pillow-Fehler bei schmalen/hohen Radien.
        mid_top = y1 + radius
    ...
    if outline is not None and width > 0:
        ...
        try:
            ...
            # Alte Pillow-Versionen kennen width bei arc/line eventuell nicht überall.
