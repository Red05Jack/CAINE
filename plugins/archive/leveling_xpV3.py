...
except Exception:  # Pillow ist optional; Text-Fallback bleibt erhalten.
    Image = None
    ImageDraw = None
    ImageFilter = None
    ImageFont = None
...
except Exception:  # Falls die Host-Umgebung keine discord.py-Dateiklasse bereitstellt.
    _discord = None

PLUGIN = {
    "name": "Leveling XP",
    "description": "XP- und Levelsystem mit getrennten Konten für Nachrichten, Voice, Invites und manuelle XP. Enthält Rank-/Admin-Befehle, Levelkurve, Snapshots und kleine digitale Manege für Fortschritt. Rankkarten werden als Bilddatei erzeugt."
}
...
async def setup_plugin(api):
    ...
    def _hex_to_rgb(value, fallback=(0, 159, 161)):
    ...
    def _fit_font(draw, text, start_size, max_width, bold=False, min_size=18):
        size = int(start_size)
        while size > min_size:
            fnt = _font(size, bold=bold)
            box = _text_bbox(draw, (0, 0), text, fnt)
            if box[2] - box[0] <= max_width:
                return fnt
            size -= 2
        return _font(min_size, bold=bold)

    def _draw_right_round_rect(draw, xy, radius, fill):
        x1, y1, x2, y2 = xy
        radius = max(0, int(radius))
        draw.rectangle((x1, y1, x2 - radius, y2), fill=fill)
        draw.rounded_rectangle((x1, y1, x2, y2), radius=radius, fill=fill)
        draw.rectangle((x1, y1, x1 + radius, y2), fill=fill)

    def _make_right_round_mask(width, height, radius):
        mask = Image.new("L", (width, height), 0)
        d = ImageDraw.Draw(mask)
        d.rectangle((0, 0, width - radius, height), fill=255)
        d.rounded_rectangle((0, 0, width, height), radius=radius, fill=255)
        d.rectangle((0, 0, radius, height), fill=255)
        return mask
    ...
    def _avatar_url(member):
        for attr in ("display_avatar", "avatar"):
            ...
            try:
                ...
                if hasattr(avatar, "with_format"):
                    avatar = avatar.with_format("png")
            except Exception:
                pass
        ...
        url = getattr(default_avatar, "url", None)
        return str(url) if url else None

    async def _fetch_avatar_bytes(member):
        url = _avatar_url(member)
        if not url or not url.startswith(("http://", "https://")):
            return None

        def _download():
            req = urllib.request.Request(url, headers={"User-Agent": "LevelingXP-RankCard/1.0"})
    ...
    def _safe_filename_part(value):
        value = re.sub(r"[^A-Za-z0-9_.-]+", "_", str(value or "rank"))
        return value[:48] or "rank"

    def _build_rank_card_png_sync(member_name, avatar_bytes, missing_xp, rank, level, progress, next_cost, accent_hex):
        if Image is None:
            return None
        accent = _hex_to_rgb(accent_hex)
        glow = tuple(min(255, int(c * 1.05 + 10)) for c in accent)
        canvas_w, canvas_h = 1080, 220
        card_x, card_y = 78, 25
        card_w, card_h = 980, 170
        radius = 86
        img = Image.new("RGBA", (canvas_w, canvas_h), (0, 0, 0, 0))

        # Card-Maske im Stil der rank.html: links offen/rechte Rundung.
        card_layer = Image.new("RGBA", (card_w, card_h), (5, 7, 12, 255))
        card_mask = _make_right_round_mask(card_w, card_h, radius)

        # Leuchtender Fortschrittsbalken.
        percent = 0.0 if next_cost <= 0 else max(0.0, min(1.0, float(progress) / float(next_cost)))
        bar_w = max(1, int(card_w * percent))
        bar_layer = Image.new("RGBA", (card_w, card_h * 3), (0, 0, 0, 0))
        bd = ImageDraw.Draw(bar_layer)
        bd.rectangle((0, 0, bar_w, card_h * 3), fill=(*glow, 230))
        bar_layer = bar_layer.filter(ImageFilter.GaussianBlur(26))
        cropped = bar_layer.crop((0, card_h, card_w, card_h * 2))
        card_layer.alpha_composite(cropped, (0, 0))

        # Dunkler Lesestreifen.
        strip = Image.new("RGBA", (card_w, card_h), (0, 0, 0, 0))
        sd = ImageDraw.Draw(strip)
        _draw_right_round_rect(sd, (72, 21, card_w - 24, 143), 64, (0, 0, 0, 128))
        card_layer.alpha_composite(strip, (0, 0))

        img.paste(card_layer, (card_x, card_y), card_mask)

        d = ImageDraw.Draw(img)
        # Rand: oben/unten/rechts, links bewusst ohne Border.
        border = (*accent, 255)
        lw = 4
        d.line((card_x, card_y + lw // 2, card_x + card_w - radius, card_y + lw // 2), fill=border, width=lw)
        d.line((card_x, card_y + card_h - lw // 2, card_x + card_w - radius, card_y + card_h - lw // 2), fill=border, width=lw)
        d.arc((card_x + card_w - 2 * radius, card_y, card_x + card_w, card_y + 2 * radius), 270, 360, fill=border, width=lw)
        d.arc((card_x + card_w - 2 * radius, card_y + card_h - 2 * radius, card_x + card_w, card_y + card_h), 0, 90, fill=border, width=lw)
        d.line((card_x + card_w - lw // 2, card_y + radius, card_x + card_w - lw // 2, card_y + card_h - radius), fill=border, width=lw)

        # Avatar-Kreis.
        avatar_size = 180
        avatar_x = card_x - 78
        avatar_y = card_y + card_h // 2 - avatar_size // 2
        avatar_img = None
        if avatar_bytes:
            try:
                avatar_img = Image.open(io.BytesIO(avatar_bytes)).convert("RGBA")
                avatar_img = ImageOps.fit(avatar_img, (avatar_size, avatar_size), method=Image.LANCZOS, centering=(0.5, 0.5))
            except Exception:
                avatar_img = None
        if avatar_img is None:
            avatar_img = Image.new("RGBA", (avatar_size, avatar_size), (17, 17, 17, 255))
            ad = ImageDraw.Draw(avatar_img)
            ad.ellipse((0, 0, avatar_size - 1, avatar_size - 1), fill=(17, 17, 17, 255))
            ad.ellipse((42, 30, 138, 126), fill=(*accent, 220))
            ad.ellipse((25, 112, 155, 205), fill=(*accent, 160))
        circle = Image.new("L", (avatar_size, avatar_size), 0)
        cd = ImageDraw.Draw(circle)
        cd.ellipse((0, 0, avatar_size - 1, avatar_size - 1), fill=255)
        img.paste(avatar_img, (avatar_x, avatar_y), circle)

        # Texte im HTML-Layout.
        name_text = str(member_name or "Unbekannt")[:32]
        xp_text = f"Noch {compact_xp(missing_xp)} XP"
        rank_label = "RANG"
        level_label = "LEVEL"
        rank_value = f"#{rank}"
        level_value = str(level)

        name_font = _fit_font(d, name_text, 42, 500, bold=True, min_size=24)
        xp_font = _fit_font(d, xp_text, 32, 500, bold=False, min_size=20)
        label_font = _font(30, bold=False)
        value_font = _font(44, bold=True)

        d.text((card_x + 135, card_y + 32), name_text, font=name_font, fill=(255, 255, 255, 255))
        d.text((card_x + 135, card_y + 93), xp_text, font=xp_font, fill=(255, 255, 255, 150))

        parts = [(rank_label, label_font, (255, 255, 255, 155)), (rank_value, value_font, (255, 255, 255, 255)), (level_label, label_font, (255, 255, 255, 155)), (level_value, value_font, (255, 255, 255, 255))]
        gap = 8
        widths = []
        heights = []
        for text, fnt, _fill in parts:
            box = _text_bbox(d, (0, 0), text, fnt)
            widths.append(box[2] - box[0])
            heights.append(box[3] - box[1])
        total_w = sum(widths) + gap * (len(parts) - 1)
        x = card_x + card_w - 58 - total_w
        baseline_y = card_y + 58
        for idx, (text, fnt, fill) in enumerate(parts):
            box = _text_bbox(d, (0, 0), text, fnt)
            y = baseline_y + (44 - (box[3] - box[1])) // 2
    ...
    async def build_rank_card_png(member, account, rank, level, progress, next_cost, missing):
        if Image is None:
            return None
        ...
        return await loop.run_in_executor(
            ...
            int(progress),
            int(next_cost),
            normalize_color(account.get("rank_color")),
        )

    async def send_ctx_file(ctx, content, filename, data):
        if not data:
            return False
        channel = get_ctx_channel(ctx)
        if channel is None or not hasattr(channel, "send"):
            return False
        try:
            if _discord is not None and hasattr(_discord, "File"):
                file_obj = _discord.File(io.BytesIO(data), filename=filename)
                await channel.send(content=content, file=file_obj)
                return True
        except Exception:
            pass
        # Einige Wrapper unterstützen file=BytesIO direkt; wir versuchen es als zweite kleine Manege.
        try:
            bio = io.BytesIO(data)
            bio.name = filename
            await channel.send(content=content, file=bio)
            return True
        except Exception:
            return False

    async def send_rank_card_file(ctx, member, account, rank, level, progress, next_cost, missing):
        try:
            png = await build_rank_card_png(member, account, rank, level, progress, next_cost, missing)
            filename = f"rank_{_safe_filename_part(display_name(member))}_{getattr(member, 'id', 'user')}.png"
            return await send_ctx_file(ctx, "🎪 **Rank-Karte** – frisch aus der digitalen Manege!", filename, png)
        except Exception as exc:
            await emit_log(f"[RankCard] {type(exc).__name__}: Bildkarte konnte nicht erzeugt werden.")
            return False
    ...
    async def send_rank(ctx, member, debug=False):
        ...
        if debug:
            ...
            return
        sent_image = await send_rank_card_file(ctx, member, account, rank, level, progress, next_cost, missing)
        if sent_image:
            return
        await reply(ctx,
            ...
            f"_Die Bildmaschine konnte nicht feuern. Installiere `Pillow` und stelle sicher, dass der Bot Dateien senden darf._"
        )
    ...
    @api.command({"names": ["xp"], "description": "Zeigt deine XP und dein Level als Rankkarten-Bild an. Optional: xp @User", "level": "user"})
    ...
    @api.command({"names": ["level"], "description": "Zeigt deine XP und dein Level als Rankkarten-Bild an. Optional: level @User", "level": "user"})
    async def level_command(ctx, args):
        await xp_command(ctx, args)

    @api.command({"names": ["myrank"], "description": "Zeigt deine Rank-Karte als Bild. Optional: myrank @User [debug]", "level": "user"})
    ...
    @api.command({"names": ["set-rank-color"], "description": "Setzt deine Rank-Farbe. Nutzung: set-rank-color #FFFFFF", "level": "user"})
    ...
    @api.command({"names": ["addxp"], "description": "Fügt einem Nutzer Manual-XP hinzu. Nutzung: addxp @User <Menge>", "level": "admin"})
    async def addxp_command(ctx, args):
        await manual_xp_command(ctx, args, +1)

    @api.command({"names": ["givexp"], "description": "Fügt einem Nutzer Manual-XP hinzu. Nutzung: givexp @User <Menge> [Grund]", "level": "admin"})
    async def givexp_command(ctx, args):
        await manual_xp_command(ctx, args, +1)

    @api.command({"names": ["removexp"], "description": "Entfernt einem Nutzer Manual-XP. Nutzung: removexp @User <Menge> [Grund]", "level": "admin"})
    async def removexp_command(ctx, args):
        await manual_xp_command(ctx, args, -1)

    @api.command({"names": ["xp-liste"], "description": "Gibt die aktuelle XP-Liste aus. Erfordert Server verwalten.", "level": "admin"})
    ...
    @api.command({"names": ["einladungen-nachbearbeiten"], "description": "Schaltet pending Invite-Rewards frei und prüft sofort. Erfordert Server verwalten.", "level": "admin"})
    ...
    @api.command({"names": ["recalculate"], "description": "Rekalkuliert gespeicherte Message-XP/Invites. Nutzung: recalculate all|messages|invites", "level": "admin"})
    ...
    @api.command({"names": ["importdb"], "description": "Importiert bot-db.json Snapshot-Anhänge. Nur Bot-Master.", "level": "kinger"})
    ...
    @api.command({"names": ["help"], "description": "Zeigt XP-Bot-Befehle an.", "level": "user"})
    async def help_command(ctx, args):
        admin = is_bot_master(ctx)
        manage = has_manage_permission(ctx)
        lines = [
            "🎪 **Leveling XP – Befehle**",
            "`!myrank` / `xp` / `level` – eigene XP als Rankkarten-Bild anzeigen",
            "`!myrank @User` – Rank eines Mitglieds als Bild anzeigen",
