...
PLUGIN = {
    "name": "Level-Manege",
    "description": "XP-, Level- und Rankkarten-Apparat nach Red__Jacks Spezifikation: /rank, /levles, Rank-Farbe, manuelle XP und Recalculate-Werkzeuge."
}
...
# Konservierte HTML-Vorlage als Referenz im Plugin selbst. Die PNG-Ausgabe wird mit Pillow
# nachgebaut, weil Discord keine HTML-Datei direkt als Bild rendern kann.
RANK_HTML_TEMPLATE = """<!DOCTYPE html>
<html lang=\"de\">
<head><meta charset=\"UTF-8\" /><title>Rank Card</title>
<style>
.rank-card{--progress:88%;position:relative;width:980px;height:170px;overflow:visible}.card-mask{position:absolute;inset:0;border:4px solid #009fa1;border-left:none;border-radius:0 86px 86px 0;background:#05070c;overflow:hidden}.bar{position:absolute;left:0;top:-100%;width:var(--progress);height:300%;background:#069f9f;filter:blur(26px)}.readability-strip{position:absolute;left:72px;right:24px;top:21px;height:122px;border-radius:0 64px 64px 0;background:rgba(0,0,0,.5)}.avatar-wrap{position:absolute;left:-78px;top:50%;width:180px;height:180px;transform:translateY(-50%);border-radius:50%;overflow:hidden;background:#111}.player-name{position:absolute;left:135px;top:38px;font-size:42px;font-weight:700;color:#fff}.xp-text{position:absolute;left:135px;top:98px;font-size:32px;color:rgba(255,255,255,.55)}.rank-info{position:absolute;right:58px;top:50%;transform:translateY(-50%);display:flex;align-items:baseline;gap:8px}.rank-label{font-size:30px;letter-spacing:2px;color:rgba(255,255,255,.6)}.rank-value,.level-value{font-size:44px;font-weight:700;color:#fff}
</style></head><body><div class=\"rank-card\"><div class=\"card-mask\"><div class=\"bar\"></div><div class=\"readability-strip\"></div><div class=\"player-name\">{username}</div><div class=\"xp-text\">{xp_text}</div><div class=\"rank-info\"><span class=\"rank-label\">RANG</span><span class=\"rank-value\">#{rank}</span><span class=\"rank-label\">LEVEL</span><span class=\"level-value\">{level}</span></div></div><div class=\"avatar-wrap\"></div></div></body></html>"""
...
def calculate_message_xp(message_id: int, min_xp: int = _MESSAGE_XP_MIN, max_xp: int = _MESSAGE_XP_MAX) -> int:
    # SplitMix64, identisch zur Spezifikation in Geist und Zahlenraum.
    mask = (1 << 64) - 1
...
def _parse_user_id(tokens: List[str]) -> Optional[int]:
    if not tokens:
        return None
    match = _MENTION_RE.match(tokens[0])
    if match:
        return int(match.group(1))
    if _ID_RE.match(tokens[0]):
        return int(tokens[0])
    return None


def _is_debug_token(token: str) -> bool:
    return token.lower() in {"debug", "--debug", "-d", "true", "yes", "ja", "1"}
...
def _member_has_kinger(member: Any) -> bool:
    # S1: Kinger-Rolle. Wir akzeptieren Rollenname "Kinger" oder "Bot-Master".
...
def _is_xp_excluded_command(content: str) -> bool:
    ...
    return name in {
        "help", "levles", "levels", "rank", "set-rank-color", "set-rank-colour",
        "remove-xp", "removexp", "give-xp", "givexp", "importdb", "recalculate",
        "xp-liste", "einladungen-nachbearbeiten"
    }
...
def _ranked_users(guild_state: Dict[str, Any], guild: Any = None) -> List[Tuple[int, Dict[str, Any], int]]:
    ...
    for uid_str, user in guild_state.get("users", {}).items():
        ...
        if guild is not None:
            ...
            if member is None:
                # Rang wird bevorzugt unter aktuellen Mitgliedern berechnet. Falls der Cache
                # das Mitglied nicht kennt, behalten wir es als Fallback drin.
                pass
...
def _rank_for_user(guild_state: Dict[str, Any], user_id: int, guild: Any = None) -> int:
    for idx, (uid, _user, _total) in enumerate(_ranked_users(guild_state, guild), start=1):
        if uid == int(user_id):
            return idx
    return max(1, len(_ranked_users(guild_state, guild)) + 1)
...
def _rounded_right_rectangle(draw: Any, xy: Tuple[int, int, int, int], radius: int, fill: Any = None, outline: Any = None, width: int = 1) -> None:
    ...
    draw.rectangle((x2 - radius, y1, x2, y2), fill=fill)
    draw.pieslice((x2 - 2 * radius, y1, x2, y1 + 2 * radius), 270, 360, fill=fill)
    draw.pieslice((x2 - 2 * radius, y2 - 2 * radius, x2, y2), 0, 90, fill=fill)
    draw.rectangle((x2 - radius, y1 + radius, x2, y2 - radius), fill=fill)
    if outline is not None and width > 0:
        for i in range(width):
            draw.line((x1, y1 + i, x2 - radius, y1 + i), fill=outline)
            draw.arc((x2 - 2 * radius, y1 + i, x2 - i, y1 + 2 * radius), 270, 360, fill=outline, width=1)
            draw.line((x2 - i, y1 + radius, x2 - i, y2 - radius), fill=outline)
            draw.arc((x2 - 2 * radius, y2 - 2 * radius, x2 - i, y2 - i), 0, 90, fill=outline, width=1)
            draw.line((x1, y2 - i, x2 - radius, y2 - i), fill=outline)
...
def _make_rank_card_png(username: str, rank: int, level: int, progress: int, needed: int, total_xp: int, color: str, avatar_bytes: Optional[bytes]) -> bytes:
    if Image is None or ImageDraw is None:
        # Minimaler PNG-Fallback: 1x1 transparent PNG, falls Pillow fehlt.
    ...
    W, H = 1062, 180
    card_x, card_y, card_w, card_h = 80, 5, 980, 170
    ...
    _rounded_right_rectangle(bd, (card_x, card_y, card_x + card_w, card_y + card_h), 86, fill=(5, 7, 12, 255))

    # Progress glow, wie .bar mit Blur.
    ratio = 0.0 if needed <= 0 else max(0.0, min(1.0, progress / float(needed)))
    glow_w = int(card_w * ratio)
    glow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow)
    gd.rectangle((card_x, card_y - 170, card_x + glow_w, card_y + card_h + 170), fill=(accent[0], accent[1], accent[2], 220))
    glow = glow.filter(ImageFilter.GaussianBlur(26))
    img.alpha_composite(base)
    img.alpha_composite(glow)

    # Mask dark overlay to preserve HTML look.
    draw = ImageDraw.Draw(img)
    _rounded_right_rectangle(draw, (card_x, card_y, card_x + card_w, card_y + card_h), 86, fill=None, outline=(accent[0], accent[1], accent[2], 255), width=4)
    # Open left border: cover it with background card color.
    draw.line((card_x, card_y + 4, card_x, card_y + card_h - 4), fill=(5, 7, 12, 255), width=8)

    # Readability strip.
    _rounded_right_rectangle(draw, (card_x + 72, card_y + 21, card_x + card_w - 24, card_y + 143), 64, fill=(0, 0, 0, 128))

    # Avatar circle at HTML coordinates: left -78 relative to card -> x=2.
    av_x, av_y, av_s = 2, 0, 180
    ...
    img.alpha_composite(avatar, (av_x, av_y), mask)
    ...
    xp_text = f"{compact_number(progress)} / {compact_number(needed)} XP"
    if needed > progress:
        xp_text = f"Noch {compact_number(needed - progress)} XP"
    draw.text((card_x + 135, card_y + 93), xp_text, font=f_xp, fill=(255, 255, 255, 145))

    # Rank info right side.
    labels_values = [("RANG", f"#{rank}"), ("LEVEL", str(level))]
    x = card_x + card_w - 58
    y_label = card_y + 70
    # draw from right to left for stable alignment
    parts = []
    for label, val in labels_values:
        parts.extend([(label, f_label, (255, 255, 255, 155)), (val, f_value, (255, 255, 255, 255))])
    widths = []
...
async def _maybe_send_levelup(api: Any, ctx_or_message: Any, user_id: int, movement: Dict[str, Any]) -> None:
    if not movement.get("applied") or int(movement.get("new_level", 0)) <= int(movement.get("old_level", 0)):
        return
    guild = _guild_from_ctx(ctx_or_message)
    ...
    try:
        ...
        # Kein Secret-Leak, nur höflicher Hinweis im Log falls vorhanden.
        try:
...
async def setup_plugin(api):
    @api.command({
        "names": ["levles", "levels"],
        "description": "Zeigt den Level-Baum und die XP-Kosten. Alias: /levles nach Vorgabe.",
        "level": "user",
        "options": []
    })
    async def levels_command(ctx, args):
        ...
        if guild_id is None or author is None:
            await api.reply(ctx, "Diese Attraktion funktioniert nur auf einem Server, nicht im Nebel der Direktnachrichten.")
            return
        ok, wait = _cooldown_ok("levels", guild_id, int(author.id), 10)
        if not ok:
            await api.reply(ctx, f"Langsam, kleiner Level-Akrobat! Warte noch {wait}s.")
            return
        lines = ["**Level-Baum der digitalen Manege**"]
        cumulative = 0
        for lvl in range(0, 25):
            cost = xp_for_next_level(lvl)
            cumulative += cost
            lines.append(f"Level {lvl} → {lvl + 1}: **{cost} XP**  | Gesamt bis dahin: **{cumulative} XP**")
        lines.append("\nFormel: `round(20 * (Level + 1)^1.9)`. Konfetti exakt, Chaos kontrolliert!")
        await api.reply(ctx, "\n".join(lines))
    ...
    async def rank_command(ctx, args):
        ...
        png = _make_rank_card_png(username, rank, level, progress, needed, total, user.get("rank_color", _DEFAULT_COLOR), avatar_bytes)
        await _send_rank_card(ctx, api, png)

    @api.command({
        "names": ["set-rank-color", "set-rank-colour"],
        "description": "Setzt deine Rank-Karten-Farbe, z. B. /set-rank-color #FFFFFF.",
        "level": "user",
        "options": [
            {"name": "color", "description": "Hex-Farbe wie #FFFFFF.", "type": "string", "required": True}
        ]
    })
    async def set_rank_color_command(ctx, args):
        ...
        if not parts:
            await api.reply(ctx, "Bitte gib eine Farbe an: `/set-rank-color #FFFFFF` — sechs Hex-Zeichen, kein fauler Zauber.")
            return
    ...
    @api.command({
        ...
        "description": "S2: Gibt einem Nutzer manuelle XP: /give-xp @user amount [Grund].",
        "level": "admin",
        "options": [
    ...
    @api.command({
        ...
        "description": "S2: Entfernt manuelle XP: /remove-xp @user amount [Grund].",
        "level": "admin",
        "options": [
    ...
    @api.command({
        "names": ["importdb"],
        "description": "S1: Importiert einen bot-db.json Snapshot aus Nachrichten-Anhängen.",
        "level": "kinger",
        "options": [
            {"name": "file", "description": "bot-db.json Snapshot.", "type": "attachment", "required": True}
        ]
    })
    ...
    @api.command({
        "names": ["recalculate"],
        "description": "S1: /recalculate all|Messages|invites. Messages scannt erreichbare Channel-History neu.",
        "level": "kinger",
        "options": [
            {"name": "mode", "description": "all, messages oder invites.", "type": "string", "required": True}
        ]
    })
    async def recalculate_command(ctx, args):
        ...
        for channel in channels:
            ...
            try:
                async for msg in channel.history(limit=None, oldest_first=True):
                    ...
                    if author_msg is None:
                        continue
                    if getattr(author_msg, "bot", False):
                        continue
        ...
        async with _STATE_LOCK:
            ...
            # Ersetzt nur MessageXp; Voice/Invite/Manual bleiben erhalten.
            for uid, user in gs.get("users", {}).items():
                user["message_xp"] = 0
